...
const memesLimit = 1000000000; // Number of memes to fetch at once
...
async function fetchMemes(subredditName) {
  try {
    const response = await axios.get(`https://www.reddit.com/r/${subredditName}/top.json?sort=new&t=week&limit=${memesLimit}`);
...
if (!subredditName) return message.channel.send("Not found")
    ...
    const embed = new MessageEmbed()
      ...
      .setTitle(`${meme.title}`).setURL(meme.url)
      .setImage(`${meme.url}`)
      .setFooter(`👍 ${meme.ups}`)

    // Create the action row with buttons
    const row = new MessageActionRow().addComponents(
   //    new MessageButton().setCustomId('previous').setLabel('Previous').setStyle('PRIMARY').setEmoji("◀️"),
   
      new MessageButton().setCustomId('next').setLabel('Next').setStyle('PRIMARY').setEmoji("▶️"),
 ...
 // Collect buttons for 1 minute
    ...
    collector.on('collect', async interaction => {
      ...
      embed.setTitle(nextMeme.title).setURL(meme.url)
        embed.setImage(nextMeme.url);
        embed.setFooter(`👍 ${nextMeme.ups}`);
