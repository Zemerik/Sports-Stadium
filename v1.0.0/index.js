...
app.get("/", (req, res) => {
  res.send(
    `<!DOCTYPE html>
    <script>
      location.href = "https://zemerik.is-a.dev/Sports-Stadium"
      `
  );
}) 
...
/*fs.readdirSync("./commands/").forEach((direc) => {
const slashCommands = fs.readdirSync(`./commands/${direc}`).filter(file => file.endsWith(".js"))
for(file of slashCommands) {
  const scommandName = file.split(".")[0]
  let scommand = require(`./commands/${direc}/${commandName}`)
  client.slashCommands.set(scommandName, scommand)
}
})
*/
...
client.on('messageCreate', async (message) => {
     //  const cooldownKey = `cooldown_${message.author.id}`;
 // const cool = db.get(cooldownKey)
    //  console.log(cool)
  //
  
  if (message.content.includes(`<@&1019550024172048404>`)) {
    const pinged = new MessageEmbed()
    .setTitle('Kindly do not ping admins')
    .addFields('Need to report a problem: Use my **ssreport** command', 'Suggestion: <#1019922567269720134>')
    .setFooter('Repition of this can result in mute')
    .setTimestamp()
    message.channel.send( { embeds : [  pinged ] } )
  }

  if (message.content === 'sslist') {
    const embed = new MessageEmbed()
